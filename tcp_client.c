...
//function to encrypt touch sensor once return pointer to ciphertext

unsigned char* encryptsensor()
{
	int i;
    uint16_t touch_value;
    	for (int i = 0; i < TOUCH_PAD_MAX; i++) {
...
static void tcp_client_task(void *pvParameters)
{
	...
	unsigned char *startcipher;
    	startcipher=encryptsensor();
    	printf("SEND : CIPHERTEXT with TAG:");
    	for (i=0; i <clen; ++i) {
        	cipher[i]=*(startcipher+i);  
         	printf("%02x ",cipher[i]);
	}
        printf("\n");
