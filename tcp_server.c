 ...
 */
unsigned char* tp_example_read_task(void *pvParameter)
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            // Data received
                ...
                //ESP_LOGI(TAG, "DAPAT: %s", rx_buffer);
                ESP_LOGI(TAG, "DAPAT:");
		int i;
		for (i=0;i<24;++i){
		...
		printf("\n");
	 	...
	 	//crypto_aead_decrypt(m_dec, &mlen_dec, NULL, rx_buffer, 24, ad, adlen, npub, k); 
	 	crypto_aead_decrypt(m_dec, &mlen_dec, NULL, rx_buffer, 24, ad, adlen, npub, k); 
                 printf("\nDECRYPT %llu bytes: ",mlen_dec);
                 for (i = 0; i <mlen_dec; ++i) {
                        printf("%02x ", m_dec[i]);
                 }
                printf("\n");
